...
const Link: React.FC<link> = (props) => {
    ...
    return <div style={{paddingLeft:(index+1)*8}} onClick={()=>click(name)} className={`${hot ? styles.hot : ''} ${styles.name}`}>{name}</div>
}
...
const Menu: React.FC<any> = (props) => {
    ...
    return <>
        ...
        {
            hot ? <div className={styles.menu_children}>
                {children}
            </div> : ''
        }
...
const Nav_com = () => {
    ...
    const menu_link_click = (name:string)=>{
        let data = nav_list;
    ...
    const getMenu = (menu: menu[]): any => {
        return menu.map(item => {
            if (!item.children) {
                return <Link hot={item.hot} index={item.index} key={item.name} click={menu_link_click} link={item.path} name={item.name}></Link>
            } else {
